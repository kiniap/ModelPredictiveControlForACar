 ...
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    for (size_t i = 0; i < N-1; ++i) {
      ...
      // Only consider that actuation at time t
      AD<double> delta0 = vars[delta_start+i];
      AD<double> a0 = vars[a_start+i];

      const AD<double> x0_squared = x0*x0;
      const AD<double> x0_cubed = x0_squared*x0;
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  // Set the steering angle actuator limits to [-25,25] deg or [-0.436332, 0.436332] rad
    vars_upperbound[i] = 0.436332;
  }

  // Set the upper and lower acceleration/deceleration limits
  ...
  // steering angle and acceleration actuation are the 1st two elements
  result.push_back(solution.x[delta_start]);
  result.push_back(solution.x[a_start]);


  // Push back the future x and y to display the green line
  for(size_t i = 0;i < N-1; ++i){
